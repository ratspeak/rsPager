...
bool SDStore::wipeRatpager() {
    if (!_ready) return false;
    Serial.println("[SD] Wiping /ratpager/ ...");
    wipeDir(SD_PATH_MESSAGES);
    wipeDir(SD_PATH_CONTACTS);
    wipeDir("/ratpager/identity");
    wipeDir(SD_PATH_CONFIG_DIR);
    wipeDir(SD_PATH_TRANSPORT);
    SD.rmdir(SD_PATH_ROOT);
    Serial.println("[SD] Wipe complete, recreating dirs...");
    return formatForRatpager();
}
...
bool SDStore::formatForRatpager() {
    ...
    ok &= ensureDir("/ratpager/identity");
